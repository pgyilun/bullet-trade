...
示例:
  ...
  更多信息请访问: https://github.com/BulletTrade/bullet-trade
    ...
    parser.add_argument("--env-file", dest="env_file", default=None, help="指定要加载的 .env 文件（优先于默认搜索）")
    ...
    backtest_parser.add_argument("--start", type=str, required=True, help="回测开始日期 (YYYY-MM-DD)")
    backtest_parser.add_argument("--end", type=str, required=True, help="回测结束日期 (YYYY-MM-DD)")
    backtest_parser.add_argument("--cash", type=float, default=100000, help="初始资金 (默认: 100000)")
    ...
    backtest_parser.add_argument(
        "--output", type=str, default="./backtest_results", help="输出目录 (默认: ./backtest_results)"
    )
    ...
    backtest_parser.add_argument(
        "--no-html", dest="generate_html", action="store_false", help="不生成交互式HTML报告 (默认生成)"
    )
    backtest_parser.add_argument(
        "--no-logs", dest="generate_logs", action="store_false", help="仅在终端打印日志，不写日志文件 (默认写入输出目录)"
    )
    backtest_parser.add_argument(
    ...
    backtest_parser.add_argument("--auto-report", action="store_true", help="回测完成后自动生成标准化报告")
    backtest_parser.add_argument(
        "--report-format", choices=["html", "pdf"], default="html", help="自动报告格式 (默认: html)"
    )
    backtest_parser.add_argument("--report-template", type=str, default=None, help="自定义报告模板路径")
    backtest_parser.add_argument(
        "--report-metrics", type=str, default=None, help="报告中展示的指标名称列表（逗号分隔）"
    )
    backtest_parser.add_argument("--report-title", type=str, default=None, help="报告标题（默认使用输出目录名称）")
    backtest_parser.add_argument(
        "--report-output",
        type=str,
        default=None,
    ...
    optimize_parser.add_argument("--processes", type=int, default=None, help="并行进程数 (默认: CPU核心数)")
    ...
    report_parser.add_argument("--output", "-o", type=str, default=None, help="输出文件路径（默认写入回测目录）")
    ...
    report_parser.add_argument("--metrics", type=str, default=None, help="展示的指标名称，使用逗号分隔")
    ...
    server_parser.add_argument("--tls-cert", dest="tls_cert", default=None, help="TLS 证书路径（可选）")
    server_parser.add_argument("--tls-key", dest="tls_key", default=None, help="TLS 私钥路径（可选）")
    server_parser.add_argument(
        "--enable-data", dest="enable_data", action="store_true", default=None, help="强制启用数据服务"
    )
    server_parser.add_argument(
        "--disable-data", dest="enable_data", action="store_false", help="强制禁用数据服务"
    )
    server_parser.add_argument(
        "--enable-broker", dest="enable_broker", action="store_true", default=None, help="强制启用券商服务"
    )
    server_parser.add_argument(
    ...
    server_parser.add_argument(
        "--log-file", dest="log_file", default=None, help="将服务器日志写入指定文件（默认仅输出到控制台）"
    )
    server_parser.add_argument(
    ...
    server_parser.add_argument(
        "--access-log", dest="access_log", action="store_true", default=None, help="显式开启请求访问日志"
    )
    server_parser.add_argument(
    ...
    lab_parser.add_argument(
        "--notebook-dir", dest="notebook_dir", default=None, help="Notebook 根目录（默认 ~/bullet-trade）"
    )
    lab_parser.add_argument(
